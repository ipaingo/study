...
def openn2(combo1, a, combo2, txt3):
	...
	if combo2.get()=='':
		...
		if a=='-1':
			...
			print(a)
			for qwe in Team.bios.find({combo1.get():{combo2.get():{a:hg}}},{"_id":0}):
				ty=str(qwe.values())+"\n"
				print(ty)
				print("eqw")
				ty=ty[11:]
...
my_button2 = Button(window, text="результат", command=lambda :openn(entry_user, combo1, txt2, combo2, txt3))
my_button2.place(x=500, y=x)

entry_user.bind("<<ComboboxSelected>>", lambda e:callback(entry_user.get(), combo1, txt2, combo2, txt3))
