...
my_button = Button(
    window,
    text="Добавить ключ-значение",
    command=lambda: add(entry_user, combo1, txt2, combo2),
)
my_button.place(x=100, y=x)

my_button1 = Button(
    window, text="Сохранить документ", command=lambda: savee(entry_user)
)
my_button1.place(x=300, y=x)

my_button2 = Button(
    window, text="Показать документы", command=lambda: openn(entry_user)
)
my_button2.place(x=500, y=x)

entry_user.bind(
    "<<ComboboxSelected>>", lambda e: callback(entry_user.get(), combo1, txt2, combo2)
)
