...
def calculate_statistics(data_stat):
    ...
    # массив значений в пятиминутном отрывке, который надо будет обработать статистике. началом является первая отметка, поэтому поместим в массив соответствующее ей значение.
