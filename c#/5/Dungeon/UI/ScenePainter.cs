...
public class ScenePainter : Canvas
{
	...
	protected override void OnPointerPressed(PointerPressedEventArgs e)
	{
		...
		var position = new Point((int)(location.X / (0.33 * cellWidth)), (int)(location.Y / (0.33 * cellHeight)));
	...
	private void DrawLevel(DrawingContext context)
	{
		...
		foreach (var chestPoint in currentMap.Chests)
			context.DrawImage(chest,
				new Rect(chestPoint.X * (0.33 * cellWidth), chestPoint.Y * (0.33 * cellHeight), (0.33 * cellWidth), (0.33 * cellHeight)));
		context.DrawImage(castle,
			new Rect(currentMap.Exit.X * (0.33 * cellWidth), currentMap.Exit.Y * (0.33 * cellHeight), (0.33 * cellWidth), (0.33 * cellHeight)));
	}

	private void DrawPath(DrawingContext context, Color color, IEnumerable<Point> path)
	{
		var points = path.Select(x =>
			new Avalonia.Point(x.X * (0.33 * cellWidth) + (0.33 * cellWidth) * 0.5f, x.Y * (0.33 * cellHeight) + (0.33 * cellHeight) * 0.5f)).ToArray();
		var newStyle = new DashStyle(new[] { (0.33 * cellWidth) * 0.125f, (0.33 * cellHeight) * 0.125f }, 1d);
		var pen = new Pen(color.ToUint32(), (0.33 * cellHeight) * 0.125f, newStyle);
	...
	private void DrawMainPath(DrawingContext context, int interation)
	{
		...
		var position = path[^1];
		context.DrawImage(peasant, new Rect(position.X * (0.33 * cellWidth), position.Y * (0.33 * cellHeight), (0.33 * cellWidth), (0.33 * cellHeight)));
	}

	private void DrawAdditionalPaths(DrawingContext context, Point lastClick)
	{
		context.FillRectangle(Brushes.Red,
			new Rect(lastClick.X * (0.33 * cellWidth), lastClick.Y * (0.33 * cellHeight), (0.33 * cellWidth), (0.33 * cellHeight)));
	...
	private void RenderMap(DrawingContext context)
	{
		...
		for (var x = 0; x < width; x++)
		{
			for (var y = 0; y < height; y++)
			{
				...
				context.DrawImage(image, new Rect(x * (0.33 * cellWidth), y * (0.33 * cellHeight), (0.33 * cellWidth), (0.33 * cellHeight)));
			}
		}
	}
}
